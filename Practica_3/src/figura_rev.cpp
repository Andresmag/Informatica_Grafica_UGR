...
void Figura_rev::GenerarFigura(vector<_vertex3f> perfil, int n){
	...
	vertices = perfil; //metemos el perfil en el vector de verices
	float angulo = (2*PI)/n; // calculamos en los trozos que hay que dividir la circunferencia
	int k = perfil.size();
	vertices.resize((n)*k); //redimensionamos el vector de vertices 
	int pos_triangles = 0;
	triangles.resize((perfil.size()-1)*n*2); //redimensionamos el vector de triangulos
	...
	//Eliminamos el ultimo amance de k
	k--;
	
	//Completamos las caras del perfil que nos faltan para unir 
	//los puntos del ultimo giro con el principio
	for(int i=1; i<perfil.size(); i++){
		triangles[pos_triangles++] = _vertex3ui(i, i-1,k-perfil.size()+i);
		triangles[pos_triangles++] = _vertex3ui(i, k-perfil.size()+i, k-perfil.size()+i+1);
	}
	...
	if(tapa_superior){
		...
		int pos_vertices = 0;
		for(int i=0; i<n; i++){
			 triangles[pos_triangles] = _vertex3ui(vertices.size()-1,pos_vertices,((pos_vertices+perfil.size()) % (vertices.size()-1)));
			 pos_triangles++;
	...
	//Añadimos la tapa superior, tanto el vertice como las caras
	if(tapa_inferior){
		int valor = triangles.size();
		triangles.resize(valor + n);
		vertices.push_back(tapa_abajo);
		int pos_vertices = perfil.size()-1;
		int modulo;
		
		//Comprobamos si la figura tiene tapa superior
		if(tapa_superior)
		...
		for(int i=0; i<n; i++){
			triangles[pos_triangles] = _vertex3ui(vertices.size()-1, ((pos_vertices+perfil.size()) % modulo),  pos_vertices);;
			pos_triangles++;
			pos_vertices += perfil.size();
		}
	}
